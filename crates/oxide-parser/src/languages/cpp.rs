...
fn traverse_node(
    ...
    let (symbol_kind, name_node) = match kind_str {
        ...
        "class_specifier" => {
            ...
            if let Some(n) = name_n
                && let Ok(cname) = n.utf8_text(content.as_bytes())
            {
                current_scope = Some(format!("class {}", cname));
            }
        ...
        "namespace_definition" => {
            let name_n = node.child_by_field_name("name");
            if let Some(n) = name_n
                && let Ok(nsname) = n.utf8_text(content.as_bytes())
            {
                current_scope = Some(format!("namespace {}", nsname));
            }
            (Some(SymbolKind::Module), name_n)
        }
        "enum_specifier" => {
            let name_n = node.child_by_field_name("name");
            if let Some(n) = name_n
        ...
        && !name.is_empty()
    {
        ...
        let doc = extract_cpp_doc_comment(content, start_point.row);
...
fn extract_cpp_doc_comment(content: &str, start_row: usize) -> Option<String> {
...
fn traverse_calls(node: Node, content: &str, symbols: &[SymbolRecord], calls: &mut Vec<CallEdge>) {
    ...
    if kind == "call_expression" {
        ...
        && let Ok(callee_text) = field_node.utf8_text(content.as_bytes())
    {
        let line = node.start_position().row + 1;
