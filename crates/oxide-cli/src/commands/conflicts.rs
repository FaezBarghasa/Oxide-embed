...
use oxide_core::memory::{ConflictDetector, MemoryKind, MemoryStatus};
...
pub async fn handle_conflicts(project_root: &Path) -> Result<()> {
    ...
    println!("⚖️  Reviewing Contradictions & Conflicts in Active Rules (Total: {})\n", rules.len());
    ...
    for i in 0..rules.len() {
        for j in (i + 1)..rules.len() {
                ...
                && ConflictDetector::is_potential_conflict(r1, &r2.content, r2.kind)
            {
                ...
                println!("  [Rule 1] [{}] `{}` ({})", r1.kind.as_str().to_uppercase(), r1.title, r1.id);
                println!("           \"{}\"", r1.content);
                println!("  [Rule 2] [{}] `{}` ({})", r2.kind.as_str().to_uppercase(), r2.title, r2.id);
                println!("           \"{}\"", r2.content);
                println!();
            }
        }
    ...
    if conflict_count == 0 {
        ...
        println!("  💡 Tip: Use `oxide-embed remember --auto-resolve` to automatically supersede older conflicting entries.");
    }
